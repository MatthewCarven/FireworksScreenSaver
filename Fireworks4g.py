# Fireworks9g.scr - ID/Error - Matthew Carven - Google Gemini
# Final Version: Added High Score Persistence.
# - Saves "Best Time" and "Max Chaser Kills" to JSON.
# - Checks and saves records automatically on death/reset.
...
DEFAULT_CONFIG = {
    ...
    "high_score_time": 0.0,   # NEW
    "high_score_chasers": 0   # NEW
}
...
def load_config():
    try:
        with open(CONFIG_PATH, 'r') as f:
            data = json.load(f)
            # Ensure all keys exist (including new high scores)
...
def save_config(config_data):
    try:
        ...
        # Silently fail in game loop to avoid popup spam, only error if critical
        pass 
...
def open_settings_window():
    ...
    # Display current high scores in settings
    ...
    def on_save():
        new_config = {
            ...
            "high_score_time": best_time,       # Preserve existing scores
    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                check_and_save_high_scores() # Save on exit
                running = False
            
            if event.type == pygame.MOUSEBUTTONDOWN:
                check_and_save_high_scores() # Save on click exit
                running = False
                
            if event.type == pygame.KEYDOWN:
                if event.key in [pygame.K_q, pygame.K_ESCAPE]:
                    check_and_save_high_scores() # Save on key exit
                    running = False
        ...
        # --- WALL HUG PENALTY ---
        if DRIFT_ACTIVE:
            check_and_save_high_scores() # Check before resetting!
            LAST_HIT_TIME = pygame.time.get_ticks() 
