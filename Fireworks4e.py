...
# --- Configuration Paths ---
# We hardcode this to a dedicated folder on the C drive to avoid 
# Windows "Program Files" or "System32" permission issues.
...
def open_settings_window():
    """Opens a Tkinter window to adjust settings."""
    ...
    tk.Checkbutton(root, text="Enable Cursor Pushers (Unruly Mode)", variable=pusher_var).pack(pady=10)
    
    # Add a label showing where the config is saved
...
class Particle:
    ...
    def move(self):
        # 1. Gravity (Everyone EXCEPT Chaser and Cursor Pusher)
    ...
    def explode(self):
        ...
        if self.is_special and self.p_type == TYPE_ROCKET:
            ...
            if self.p_type == TYPE_CHASER:
                ...
                pusher = Particle(0, 0, self.color, TYPE_CURSOR_PUSHER, vx=push_vx, vy=push_vy)
...
def main():
    ...
    while running:
        ...
        # --- EDGE CHECK LOGIC ---
        mx, my = pygame.mouse.get_pos()
        if mx <= 2 or mx >= WIDTH - 3 or my <= 2 or my >= HEIGHT - 3:
            edge_hover_timer += 1
        else:
            edge_hover_timer = 0
            
        if edge_hover_timer > EDGE_LIMIT:
            safe_x = random.randint(200, WIDTH - 200)
            safe_y = random.randint(200, HEIGHT - 200)
            pygame.mouse.set_pos(safe_x, safe_y)
            edge_hover_timer = 0
        ...
        trail_surface.set_alpha(TRAIL_ALPHA) # Used from Config
