...
class Particle:
    def __init__(self, x, y, color, p_type, vx=0, vy=0):
        ...
        if self.p_type == TYPE_SPARK or self.p_type == TYPE_TRAIL:
            ...
            self.max_speed = random.uniform(7, 10) 
    ...
    def explode(self):
        ...
        if self.is_special and self.p_type == TYPE_ROCKET:
            ...
            if self.p_type == TYPE_CHASER:
                ...
                kick_distance = 150 
                push_x = (dx / dist) * kick_distance
                push_y = (dy / dist) * kick_distance
                
                duration = random.randint(60, 90) 
                push_vx = push_x / duration
                push_vy = push_y / duration
                
                pusher = Particle(0, 0, self.color, TYPE_CURSOR_PUSHER, vx=push_vx, vy=push_vy)
                pusher.life = duration 
...
def main():
    ...
    while running:
        ...
        # --- EDGE CHECK LOGIC ---
        mx, my = pygame.mouse.get_pos()
        # Check boundaries (using small buffer of 2 pixels)
        ...
        if edge_hover_timer > EDGE_LIMIT:
            # Teleport mouse to safe random location
